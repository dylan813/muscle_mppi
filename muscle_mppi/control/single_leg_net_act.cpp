...
// is violated when NUM_MUSCLES=3 (the net-act build). This function replicates
// the same physics but takes:
//   u[NUM_JOINTS]           — net activations ∈ [-1, 1]
//   act_state[2*NUM_JOINTS] — per-muscle filter state (in/out), size always 6
//
// The physics helpers (active_force_length, force_vel, passive_force_length)
// are reused directly from muscle.h.
// -----------------------------------------------------------------------------
    ...
    double              tau_out[NUM_JOINTS])
{
    ...
    for (int j = 0; j < NUM_JOINTS; ++j) {
        // Expand net activation to agonist/antagonist targets.
        ...
        // First-order activation filter (same rate as hill_compute_torques).
        ...
        // Geometry: linear moment-arm mapping (matches muscle.h).
        ...
        // Force-length (active + secondary shoulder).
...
SingleLegNetAct::SingleLegNetAct(const std::string& task_name,
                                   const std::string& yaml_path)
    : BaseMPPI(load_task(task_name, yaml_path))
{
    muscle_ = task_.muscle;

    // Net activation u[j] ∈ [-1, 1].
    for (int j = 0; j < NUM_JOINTS; ++j) {
        action_lo_[j] = -1.0;
        action_hi_[j] =  1.0;
    }
...
double SingleLegNetAct::step_cost(const mjData* d,
                                   const double u[NUM_JOINTS],
...
double SingleLegNetAct::terminal_cost(const mjData* d)
{
    const double* fp = d->xpos + 3 * foot_body_id_;
    double dx = fp[0] - task_.foot_target[0];
    double dy = fp[1] - task_.foot_target[1];
...
double SingleLegNetAct::rollout(int s, const RobotState& state)
{
    mjData* d = data_[s];
    set_mj_state(d, state);

    // Each rollout gets a fresh copy of the activation filter state.
    ...
    for (int t = 0; t < task_.horizon; ++t) {
        // Sample net activations from the current trajectory + noise.
        double u[NUM_JOINTS];
        for (int j = 0; j < NUM_JOINTS; ++j) {
        ...
        // Apply Hill model for all substeps.
        double tau_out[NUM_JOINTS];
...
void SingleLegNetAct::update(const RobotState& state, double tau_out[NUM_JOINTS])
{
    if (!state.valid) {
        // Hold last torque: re-apply best u through Hill model.
        double u[NUM_JOINTS];
    ...
    // Convert best net activation to torque via Hill model, advancing real_act_.
    double u[NUM_JOINTS];
    for (int j = 0; j < NUM_JOINTS; ++j) u[j] = best_traj_[j];
    hill_net_act_torques(u, state.q, state.dq, muscle_, task_.dt,
                         BASELINE, real_act_, tau_out);

    // Sync rollout filter state to real execution for the next solve.
