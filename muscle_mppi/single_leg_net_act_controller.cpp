...
#include "control/single_leg_net_act.h"
...
class NetActController {
public:
    explicit NetActController(const std::string& task      = "reach_net_act",
    ...
    void Init() {
        ...
        lowstate_subscriber_->InitChannel(
            std::bind(&NetActController::LowStateHandler, this,
                      std::placeholders::_1), 1);

        control_thread_ = CreateRecurrentThreadEx(
            "net_act_ctrl", UT_CPU_ID_NONE, 2000, &NetActController::ControlLoop, this);

        mppi_thread_ = std::thread(&NetActController::MPPILoop, this);
    }
...
private:
    ...
    void MPPILoop() {
        ...
        std::cout << "State received — running net-activation MPPI (Hill model).\n\n";
...
int main(int argc, const char** argv)
{
    ...
    const std::string task      = (argc >= 3) ? argv[2] : "reach_net_act";
    ...
    NetActController ctrl(task, yaml_path);
    ctrl.Init();
