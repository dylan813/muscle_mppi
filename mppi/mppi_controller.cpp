...
// Torque control sentinel values
...
public:
    // Task name passed to MPPILocomotion — mirrors MPPI_quad's MPPI(task='stand')
...
private:
    ...
    void LowStateHandler(const void* msg) {
        ...
        state_.quat[0] = s->imu_state().quaternion()[0];  // w
        state_.quat[1] = s->imu_state().quaternion()[1];  // x
        state_.quat[2] = s->imu_state().quaternion()[2];  // y
        state_.quat[3] = s->imu_state().quaternion()[3];  // z
    ...
    void ControlLoop() {
        ...
        if (running_time_ < STANDUP_DURATION) {
            ...
            // ---- Phase 2: MPPI position-target control ----
            double q_targets[NUM_JOINTS] = {};
            mppi_.update(state_, q_targets);

            // On transition: measure actual height, print diagnostics
            if (running_time_ - dt_ < STANDUP_DURATION) {
                ...
                std::cout << "Switching to MPPI. Update: " << ms << " ms\n";
                std::cout << "  base z   = " << state_.pos[2]  << " m  (height target)\n";
                std::cout << "  quat w   = " << state_.quat[0] << "    (1.0 = upright)\n";
                std::cout << "  base vel = " << state_.vel[0]  << " " << state_.vel[1]
                          << " " << state_.vel[2] << "\n";
            }

            // Send position targets with PD gains matching rollout
            for (int i = 0; i < NUM_JOINTS; ++i) {
                low_cmd_.motor_cmd()[i].q()   = q_targets[i];
    ...
    static constexpr double dt_              = 0.02;  // 50 Hz
    static constexpr double STANDUP_DURATION = 3.0;   // seconds

    // Standing and crouched joint positions for PD stand-up phase
...
int main(int argc, const char** argv) {
    if (argc < 2)
        ChannelFactory::Instance()->Init(1, "lo");    // sim
    else
        ChannelFactory::Instance()->Init(0, argv[1]); // real robot

    std::cout << "MPPI Controller — press Enter to start\n";
    std::cin.get();

    // Task name passed directly — mirrors MPPI_quad's MPPI(task='stand')
